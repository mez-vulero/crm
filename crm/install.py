...
def add_default_fields_layout(force=False):
	...
	sidebar_fields_layouts = {
		"CRM Lead-Side Panel": {
			"doctype": "CRM Lead",
			"layout": '[{"label": "Details", "name": "details_section", "opened": true, "columns": [{"name": "column_kl92", "fields": ["job_title", "source", "lead_owner"]}]}, {"label": "Person", "name": "person_section", "opened": true, "columns": [{"name": "column_XmW2", "fields": ["salutation", "first_name", "last_name", "email", "mobile_no"]}]}]',
		},
		"CRM Deal-Side Panel": {
			"doctype": "CRM Deal",
			"layout": '[{"label": "Contacts", "name": "contacts_section", "opened": true, "editable": false, "contacts": []}, {"label": "Details", "name": "details_section", "opened": true, "columns": [{"name": "column_na2Q", "fields": ["close_date", "probability", "next_step", "deal_owner"]}]}]',
		},
		"Contact-Side Panel": {
			"doctype": "Contact",
...
def hide_organization_from_layouts():
	"""Remove organization and generic B2B field references from existing CRM Fields Layout records.

	Stripped fields: organization, organization_name, website, territory,
	annual_revenue, no_of_employees, industry. These are generic B2B fields
	that aren't relevant to the real estate workflow.
	"""
	import json
	...
	for layout_name in layout_names:
		...
		def strip_fields_from_sections(sections):
			nonlocal changed
			# Iterate over a copy so we can remove empty sections
			for section in list(sections):
				# Strip unwanted fields from each column
				for col in section.get("columns", []):
					fields = col.get("fields", [])
					for bad_field in fields_to_strip:
						if bad_field in fields:
							fields.remove(bad_field)
							changed = True

				# Remove the section entirely if all its columns are now empty
				all_fields = []
				for col in section.get("columns", []):
					all_fields.extend(col.get("fields", []))
				if (
					not all_fields
					and not section.get("contacts")
					and section.get("name") != "contact_section"
				):
					sections.remove(section)
					changed = True
					continue

				# Rename organization-related section labels/names to "Details"
				if section.get("label") in ("Organization Details", "Organization"):
					section["label"] = "Details"
					changed = True
				if section.get("name") in ("organization_details_section", "organization_section"):
					section["name"] = "details_section"
					changed = True

		# Handle tabbed structure (e.g. CRM Deal-Data Fields)
		if layout and isinstance(layout[0], dict) and "sections" in layout[0]:
			for tab in layout:
				strip_fields_from_sections(tab.get("sections", []))
		else:
			strip_fields_from_sections(layout)

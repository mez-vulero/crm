...
class PropertyInvoice(Document):
	...
	def after_insert(self):
		self._populate_from_deal()
		self._auto_create_line_item()

	def _populate_from_deal(self):
		if not self.deal or self.buyer_name:
			return
	...
	def _auto_create_line_item(self):
		if not self.payment_collection or self.get("line_items"):
			return
		pc = frappe.get_doc("Payment Collection", self.payment_collection)
		description = pc.milestone_description or f"Payment for {pc.deal}"
		self.append("line_items", {
			"description": description,
			"quantity": 1,
			"unit_price": pc.amount_received or 0,
			"amount": pc.amount_received or 0,
		})
		self.calculate_totals()

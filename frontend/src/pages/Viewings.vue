  ...
  <div class="flex flex-col overflow-y-auto p-5">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6">
      <div class="rounded-lg border p-4">
        ...
        <div class="mt-1 text-2xl font-semibold text-orange-600">{{ counts.scheduled }}</div>
      </div>
      <div class="rounded-lg border p-4">
        <div class="text-sm text-ink-gray-5">{{ __('Completed') }}</div>
        <div class="mt-1 text-2xl font-semibold text-green-600">{{ counts.completed }}</div>
      </div>
      <div class="rounded-lg border p-4">
        <div class="text-sm text-ink-gray-5">{{ __('No-Show') }}</div>
        <div class="mt-1 text-2xl font-semibold text-red-600">{{ counts.noshow }}</div>
      </div>
      <div class="rounded-lg border p-4">
        <div class="text-sm text-ink-gray-5">{{ __('Cancelled') }}</div>
        <div class="mt-1 text-2xl font-semibold text-gray-500">{{ counts.cancelled }}</div>
      </div>
    </div>
    ...
    <div class="overflow-x-auto">
      <table v-if="appointments.length" class="w-full text-left">
        ...
        <tbody>
          <tr v-for="apt in appointments" :key="apt.name" class="border-b text-base text-ink-gray-7">
...
async function scheduleViewing() {
  ...
  try {
    await call('crm.fcrm.doctype.viewing_appointment.viewing_appointment.schedule_viewing', {

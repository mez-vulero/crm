  ...
  <div v-if="doc.name" class="flex h-full overflow-hidden">
    ...
    <Resizer side="right" class="flex flex-col justify-between border-l">
      ...
      <div class="flex items-center justify-start gap-5 border-b p-5">
        <Tooltip :text="doc.lead_name || __('Contact')">
        ...
        <div class="flex flex-col gap-2.5 truncate text-ink-gray-9">
          <Tooltip :text="doc.lead_name || __('Set a Contact')">
...
const title = computed(() => {
  return doc.value?.lead_name || props.dealId
})
...
function getParsedSections(_sections) {
  ...
  return (_sections || [])
    .map((section) => {
      ...
      section.columns = section.columns.map((column) => {
        column.fields = (column.fields || []).filter((f) => {
          const fname = typeof f === 'string' ? f : f.fieldname
    ...
    .filter((section) => {
      // Drop sections that have no fields and no contacts block left

      ...
      <div v-else-if="title == 'Payments'" class="px-3 pb-3 sm:px-10 sm:pb-5">
        ...
        <div class="flex items-center justify-between mb-2">
          ...
          <div class="flex gap-2">
            <Button v-if="scheduleEdited" variant="solid" size="sm" :label="__('Save Schedule')" :loading="savingSchedule" @click="saveSchedule" />
            <Button variant="subtle" size="sm" :label="__('Add Row')" iconLeft="plus" @click="addScheduleRow" />
          </div>
        </div>
        <div v-if="paymentScheduleRows.length" class="overflow-x-auto mb-6">
          <table class="w-full text-left">
            <thead><tr class="text-sm text-ink-gray-5 border-b">
              ...
              <th class="py-2 pr-4 font-medium">{{ __('Pct %') }}</th>
              <th class="py-2 pr-4 font-medium">{{ __('Status') }}</th>
              <th class="py-2 font-medium">{{ __('Payment Date') }}</th>
            </tr></thead>
            <tbody>
              <tr v-for="(row, idx) in paymentScheduleRows" :key="row.name || idx" class="border-b text-base" :class="row.due_date && row.due_date < todayStr && row.status !== 'Paid' ? 'text-red-600' : 'text-ink-gray-7'">
                <td class="py-1.5 pr-2"><input v-model="row.milestone" class="w-full bg-transparent border-b border-transparent hover:border-ink-gray-3 focus:border-ink-blue-5 outline-none py-1 text-base" @input="scheduleEdited = true" /></td>
                <td class="py-1.5 pr-2"><input v-model="row.due_date" type="date" class="bg-transparent border-b border-transparent hover:border-ink-gray-3 focus:border-ink-blue-5 outline-none py-1 text-base" @input="scheduleEdited = true" /></td>
                <td class="py-1.5 pr-2"><input v-model.number="row.amount" type="number" class="w-24 bg-transparent border-b border-transparent hover:border-ink-gray-3 focus:border-ink-blue-5 outline-none py-1 text-base" @input="scheduleEdited = true" /></td>
                <td class="py-1.5 pr-2"><input v-model.number="row.percentage" type="number" class="w-16 bg-transparent border-b border-transparent hover:border-ink-gray-3 focus:border-ink-blue-5 outline-none py-1 text-base" @input="scheduleEdited = true" /></td>
                <td class="py-1.5 pr-2"><Badge :variant="'subtle'" :theme="row.status === 'Paid' ? 'green' : row.status === 'Overdue' ? 'red' : 'orange'" :label="row.status" size="sm" /></td>
                <td class="py-1.5">{{ row.payment_date || '-' }}</td>
              </tr>
            </tbody>
            <tfoot>
              <tr class="text-sm font-medium text-ink-gray-7">
                <td class="py-2 pr-4">{{ __('Total') }}</td>
                <td class="py-2 pr-4"></td>
                <td class="py-2 pr-4">{{ fmtAmount(scheduleTotal) }}</td>
                <td class="py-2 pr-4">{{ scheduleTotalPct }}%</td>
                <td colspan="2"></td>
              </tr>
            </tfoot>
          </table>
        </div>
        <div v-else class="text-center py-4 text-ink-gray-5 text-sm mb-4">
          {{ __('No payment schedule rows yet.') }}
          <button class="text-ink-blue-5 underline ml-1" @click="addScheduleRow">{{ __('Add one') }}</button>
        </div>
        <!-- Payment Collections -->
...
const scheduleTotalPct = computed(() =>
  paymentScheduleRows.value.reduce((sum, row) => sum + (row.percentage || 0), 0),
)

function addScheduleRow() {
  const rows = doc.value.re_payment_schedule || []
  rows.push({ milestone: '', due_date: '', amount: 0, percentage: 0, status: 'Pending', payment_date: '' })
...
async function saveSchedule() {
  ...
  try {
    await call('frappe.client.set_value', {
      ...
      value: paymentScheduleRows.value.map((r) => ({
    ...
    toast.error(err.messages?.[0] || __('Error saving schedule'))
  } finally {
